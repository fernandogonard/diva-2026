      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {roomsArray.map((room, index) => {
            const delay = prefersReducedMotion ? 0 : index * 0.2;
            return (
            <motion.div
              key={room.id}
              ...
              <div className="p-8">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-1 text-primary-600">
                    ...
                    <span className="font-semibold" aria-label={`Capacidad: ${room.capacity} personas`}>{room.capacity}</span>
                  </div>
                </div>
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {room.amenities.map((amenity, amenityIndex) => (
                      <div key={`${room.id}-${amenityIndex}`} className="flex items-center gap-2 text-sm text-secondary-600">
                        {amenity.includes('Wi-Fi') && <Wifi className="w-4 h-4 text-primary-500" aria-hidden="true" />}
                        {amenity.includes('TV') && <Car className="w-4 h-4 text-primary-500" aria-hidden="true" />}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={`service-${index}`} className="text-center">
                  ...
                  <p className="text-secondary-600 font-medium">{service.text}</p>
                </div>
              ))}
            </div>
          </div>
