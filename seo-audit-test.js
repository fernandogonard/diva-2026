...
if (audit.passed.length > 0) {
  ...
  audit.passed.forEach((item: string) => console.log(`   ✓ ${item}`));
  console.log();
}

if (audit.warnings.length > 0) {
  console.log('🟡 WARNINGS:');
  audit.warnings.forEach((item: string) => console.log(`   ⚠ ${item}`));
  console.log();
}

if (audit.critical.length > 0) {
  console.log('🔴 CRITICAL ISSUES:');
  audit.critical.forEach((item: string) => console.log(`   ✗ ${item}`));
  console.log();
}
